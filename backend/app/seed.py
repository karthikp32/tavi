...
def yelp_vendor_seed_records(use_vendor_name_login_tokens: bool = False):
    ...
    records = []
    for idx, (name, trade, city, yelp_rating, yelp_review_count) in enumerate(YELP_VENDOR_SPECS, start=1):
        state, postal_code, latitude, longitude = CITY_DETAILS[city]
        ...
        login_token = (
            vendor_name_login_token(name)
            if use_vendor_name_login_tokens
            else f"{trade_login_slugs[trade]}-{trade_position}"
        )
        task_types = (
...
def add_yelp_vendor_record(db: Session, record: dict, now: datetime) -> Vendor:
    company = Company(id=str(uuid.uuid4()), **record["company"])
    db.add(company)
    db.flush()
    ...
    for stat_data in record["task_stats"]:
        db.add(
            VendorTaskStat(
                id=str(uuid.uuid4()),
                vendor_id=vendor.id,
    ...
    for i in range(3):
        db.add(
            VendorAvailabilityBlock(
                id=str(uuid.uuid4()),
                vendor_id=vendor.id,
                starts_at=now + timedelta(days=i, hours=9),
                ends_at=now + timedelta(days=i, hours=17),
                **record["availability"],
            )
        )
...
def seed_db(db: Session, announce: bool = True):
    ...
    # 4. Vendors, task stats & availability blocks
    now = datetime.utcnow()
    for record in yelp_vendor_seed_records():
        add_yelp_vendor_record(db, record, now)
    db.commit()

    # 5. Seed starter work orders for the dashboard
    work_order_specs = [
        (
...
def migrate_yelp_vendor_seed_data(db: Session) -> int:
    ...
    for record in yelp_vendor_seed_records(use_vendor_name_login_tokens=True):
        vendor_data = record["vendor"]
        vendor_key = (
            vendor_data["name"].lower(),
            vendor_data["trade"].lower(),
            (vendor_data["city"] or "").lower(),
        )
